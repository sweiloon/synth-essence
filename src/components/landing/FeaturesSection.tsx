...
export default function FeaturesSection() {
  ...
  const Heading = shouldReduceMotion ? "h4" : motion.h4;
  const Paragraph = shouldReduceMotion ? "p" : motion.p;

  return (
    <div className="relative z-20 py-12 md:py-16 lg:py-20 max-w-7xl mx-auto">
      <div className="px-8">
        <Heading
          {...(!shouldReduceMotion && {
      ...
      <div className="relative">
        <div className="grid grid-cols-1 lg:grid-cols-6 mt-12 xl:border rounded-md border-border">
          {features.map((feature, idx) => (
            <FeatureCard key={feature.title} className={feature.className} index={idx} disableAnimation={shouldReduceMotion}>
      ...
      <div className="flex flex-row -ml-20">
        {images.map((image, idx) => (
          <div
            ...
            className="rounded-xl -mr-4 mt-4 p-1 bg-card border border-border shrink-0 overflow-hidden transition-transform duration-300 hover:z-20 hover:scale-105 hover:rotate-0"
          >
            <img
              src={image}
              alt="Avatar examples"
...
export const SkeletonFour = () => {
  return (
    <div className="relative mt-6 flex h-[260px] w-full items-center justify-center overflow-hidden md:h-[320px]">
      <div className="pointer-events-none absolute inset-0 rounded-2xl bg-gradient-to-br from-background to-background/60" />
      <Globe />
    </div>
  );
};
...
export const Globe = ({ className }: { className?: string }) => {
  ...
  useEffect(() => {
    ...
    const globe = createGlobe(canvasRef.current, {
      devicePixelRatio: Math.min(1.5, typeof window !== "undefined" ? window.devicePixelRatio : 1.5),
      width: 320 * 2,
      height: 320 * 2,
      phi: 0,
      theta: 0,
  ...
  return (
    <canvas
      ...
      className={cn("max-w-[280px] md:max-w-[320px]", className)}
    />
  );
};
